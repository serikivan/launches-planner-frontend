...
export const loginUserAsync = createAsyncThunk(
    ...
    async (credentials: { email: string; password: string }, { rejectWithValue }) => {
        try {
            const response = await auth_api.login.loginCreate(credentials, {});
...
export const logoutUserAsync = createAsyncThunk(
    ...
    async (_, { rejectWithValue }) => {
        try {
            await auth_api.logout.logoutCreate({});
            return {};
...
export const registerUserAsync = createAsyncThunk(
    ...
    async (userData: { email: string; password: string }, { rejectWithValue }) => {
        try {
            const response = await auth_api.register.registerCreate(userData);
            return response.data;
        } catch (error) {
            const axiosError = error as AxiosError<ApiError>;
...
export const updateUserAsync = createAsyncThunk(
    ...
    async (userData: { id: number; email?: string; password?: string }, { rejectWithValue }) => {
        try {
            const response = await auth_api.profile.profileUpdate(userData);
            return response.data;
        } catch (error) {
            const axiosError = error as AxiosError<ApiError>;
