...
export const SatellitePage: FC = () => {
    ...
    useEffect(() => {
        ...
        // Используем api из Api.ts вместо функции из LaunchesAPI
        api.satellites.satellitesRead(satellite_id)
          .then((response) => setSatellites(response.data))
          .catch(() => {
            setSatellites(SATELLITES_MOCK.satellites.find((satellite) => String(satellite.satellite_id) == satellite_id))
          })
      }, [satellite_id]);
